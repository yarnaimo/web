  ...
  switch (entry.type) {
    ...
    case 'qiita':
      return {
        ...
        meta: () => (
          <>
            <MetaWithIcon icon="clock">{formatDate(entry.date)}</MetaWithIcon>
...
const Activity = ({ entries }: Props) => {
  return (
    <MainLayout>
      ...
      <Container>
        <p css={{ color: color.black(0.6), fontSize: 13 }}>
          {'GitHubリポジトリ・Qiita記事を自動で反映しています (Next.js ISR)'}
        </p>
        <div>
          {entries.map(toWorkCardProps).map((props, i) => (
            <WorkCard {...props} key={i} initialVisibility={i < 10}></WorkCard>
          ))}
        </div>
      </Container>
    </MainLayout>
